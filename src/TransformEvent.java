/** Represents a transform event that occurs when a certain command is typed with a specific item which eliminates that item from the dungeon and replaces it with a new item in the dungeon. An item entry in a dungeon file can contain verbs that can be typed right before an alias of the item and can have a transform event associated with that verb for that item.
    @author zorkaholics
*/
